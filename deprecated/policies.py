...
class GoToCabinetHandlePolicy(Policy):
    """State machine: move base to approach pose (adaptive), move arm to handle, grasp, move base away, hold."""
    def __init__(self):
        self.target_pos = np.array([0.652, 0.078, 0])
        self.target_quat = np.array([0.4999998, 0.4999998, 0.4996018, 0.5003982])  # (x, y, z, w)
        ...
        self.ee_offset = 0.75  # End effector offset (same as get_end_effector_offset for pick)
        self.approach_offset = 0.10  # Approach offset (meters) to stop short of the handle
        
        # Base following parameters (from MotionPlannerPolicy)
        self.LOOKAHEAD_DISTANCE = 0.3  # 30 cm
    ...
    def step(self, obs):
        ...
        # Update target position if leftdoor_pos is available
        if 'leftdoor_pos' in obs:
            self.target_pos = obs['leftdoor_pos'].copy()

        # On first call, compute base approach pose adaptively using waypoints
        if self.initial_base is None:
            self.initial_base = base_pose.copy()
            
            # Create pick-like command for base planning
            handle_command = {
                'primitive_name': 'pick',
                'waypoints': [base_pose[:2].tolist(), self.target_pos[:2].tolist()],
                'handle_3d_pos': self.target_pos.copy()  # Store full 3D position
            }
            
            print(f"[GoToCabinetHandlePolicy] Handle detected at: {self.target_pos}")
            print(f"[GoToCabinetHandlePolicy] Creating approach command with waypoints: {handle_command['waypoints']}")
            
            # Build base command using MotionPlannerPolicy logic
            base_command = self.build_base_command(handle_command)
            if base_command:
                ...
                self.current_waypoint_idx = 1
                self.lookahead_position = None
                print(f"[GoToCabinetHandlePolicy] Base waypoints: {self.base_waypoints}")
                print(f"[GoToCabinetHandlePolicy] Target EE position: {self.target_ee_pos}")
                
                # Compute base_target_away for later use
                ...
                print("[GoToCabinetHandlePolicy] Failed to build base command")
        ...
        # During move_base, hold the arm at its current position
        if self.state == 'move_base':
            action = self.execute_base_movement(obs)
            if action is None:  # Base movement complete
                print("[GoToCabinetHandlePolicy] Base movement complete!")
                # After base finishes, set arm_target to handle position relative to new base pose, but offset by approach_offset
                ...
                # Compute approach direction in base frame
                if dist > 1e-6:
                    approach_dir = np.array([rel_x, rel_y]) / dist
                else:
                    approach_dir = np.array([1.0, 0.0])
                # Offset the arm target by approach_offset along approach_dir (stop short of handle)
                ...
                if action and 'base_pose' in action:
                    target_pose = action['base_pose']
                    print(f"[GoToCabinetHandlePolicy] Target base pose: [{target_pose[0]:.3f}, {target_pose[1]:.3f}, {target_pose[2]:.3f}]")
        ...
        # State: move_arm
            ...
            action = {
                'base_pose': base_pose.copy(),  # Hold current base position
                'arm_pos': self.arm_target.copy(),
                'arm_quat': self.quat_target.copy(),
                'gripper_pos': self.gripper_open.copy(),
            }
            pos_close = np.allclose(arm_pos, self.arm_target, atol=0.01)
            dot = np.clip(np.dot(arm_quat, self.quat_target), -1.0, 1.0)
            angle = 2 * np.arccos(np.abs(dot))
            if pos_close and angle < np.deg2rad(5):
                print("[GoToCabinetHandlePolicy] Finished moving arm to handle pose. Transitioning to grasp.")
                self.state = 'grasp'
            return action
        ...
        # State: grasp
            ...
            if np.allclose(gripper_pos, self.gripper_closed, atol=0.1):
                print("[GoToCabinetHandlePolicy] Finished grasping handle. Transitioning to move_base_away.")
                self.state = 'move_base_away'
            return action

        # State: move_base_away (using simple direct movement)
    ...
    def execute_base_movement(self, obs):
        ...
        # Check if we've reached the final waypoint
        if self.current_waypoint_idx >= len(self.base_waypoints):
            print("[GoToCabinetHandlePolicy] All waypoints completed")
            return None  # Movement complete
        
        print(f"[GoToCabinetHandlePolicy] Current waypoint index: {self.current_waypoint_idx}/{len(self.base_waypoints)}")
        print(f"[GoToCabinetHandlePolicy] Base waypoints: {self.base_waypoints}")
        ...
        while True:
            if self.current_waypoint_idx >= len(self.base_waypoints):
                self.lookahead_position = None
                print("[GoToCabinetHandlePolicy] Reached end of waypoints, no lookahead")
                break
            ...
            print(f"[GoToCabinetHandlePolicy] Waypoint {self.current_waypoint_idx}: start={start}, end={end}")
            print(f"[GoToCabinetHandlePolicy] d={d}, f={f}, t2={t2}")
            
            if t2 is not None:
                self.lookahead_position = [start[0] + t2 * d[0], start[1] + t2 * d[1]]
                print(f"[GoToCabinetHandlePolicy] Lookahead position: {self.lookahead_position}")
                break
            if self.current_waypoint_idx == len(self.base_waypoints) - 1:
                self.lookahead_position = None
                print("[GoToCabinetHandlePolicy] Last waypoint, no lookahead")
                break
            print(f"[GoToCabinetHandlePolicy] Moving to next waypoint: {self.current_waypoint_idx + 1}")
            self.current_waypoint_idx += 1
        
        # Determine target position
        if self.lookahead_position is None:
            target_position = self.base_waypoints[-1]
            print(f"[GoToCabinetHandlePolicy] Using final waypoint as target: {target_position}")
            # Check if we've reached the final position
            position_error = self.distance(base_pose[:2], target_position)
            print(f"[GoToCabinetHandlePolicy] Position error to final target: {position_error:.3f} (tolerance: {self.position_tolerance})")
            if position_error < self.position_tolerance:
                print("[GoToCabinetHandlePolicy] Reached final position within tolerance")
                return None  # Movement complete
        else:
            target_position = self.lookahead_position
            print(f"[GoToCabinetHandlePolicy] Using lookahead as target: {target_position}")
        ...
        if self.target_ee_pos is not None:
            ...
            print(f"[GoToCabinetHandlePolicy] Target EE: {self.target_ee_pos}, dx={dx:.3f}, dy={dy:.3f}, desired_heading={desired_heading:.3f}")
            ...
            heading_diff = self.restrict_heading_range(desired_heading - base_pose[2])
            target_heading += frac * heading_diff
            print(f"[GoToCabinetHandlePolicy] Heading: current={base_pose[2]:.3f}, desired={desired_heading:.3f}, diff={heading_diff:.3f}, frac={frac:.3f}, target={target_heading:.3f}")
    ...
    def dot(self, a, b):
        """Dot product helper function from MotionPlannerPolicy"""
        return a[0] * b[0] + a[1] * b[1]

    def intersect(self, d, f, r, use_t1=False):
        """Line-circle intersection from MotionPlannerPolicy"""
        # https://stackoverflow.com/questions/1073336/circle-line-segment-collision-detection-algorithm/1084899%231084899
        a = self.dot(d, d)
        b = 2 * self.dot(f, d)
        c = self.dot(f, f) - r * r
    ...
    def distance(self, pt1, pt2):
        """Calculate distance between two points from MotionPlannerPolicy"""
        return math.sqrt((pt2[0] - pt1[0])**2 + (pt2[1] - pt1[1])**2)

    def restrict_heading_range(self, h):
        """Normalize heading to [-π, π] range from MotionPlannerPolicy"""
        return (h + math.pi) % (2 * math.pi) - math.pi

    def get_end_effector_offset(self, primitive_name):
        """Calculate end-effector offset based on task and gripper state from MotionPlannerPolicy"""
        # Simplified version - assume gripper starts open
        gripper_open = True  
        if gripper_open:
            return 0.75  # Use same as self.ee_offset
        return {'toss': 1.30, 'shelf': 0.75, 'drawer': 0.80}.get(primitive_name, 0.75)
    ...
    def build_base_command(self, command):
        ...
        if new_waypoint is not None:
            ...
            # Base is too close to target end effector position and needs to back up
            print('[GoToCabinetHandlePolicy] Warning: Base needs to deviate from commanded path to reach target position, watch out for potential collisions')
            curr_position = command['waypoints'][0]
            signed_dist = self.distance(curr_position, target_ee_pos) - end_effector_offset
            dx = target_ee_pos[0] - curr_position[0]
            dy = target_ee_pos[1] - curr_position[1]
...
class GoToCabinetHandlePolicyRight(Policy):
    """State machine: move base to approach pose (adaptive), move arm to right handle, grasp, move base away, hold."""
    def __init__(self):
        # Placeholder right handle pose and orientation
        self.target_pos = np.array([0.652, -0.078, 0])
    ...
    def step(self, obs):
        ...
        if 'rightdoor_pos' in obs:
            ...
            print("[GoToCabinetHandlePolicyRight] WARNING: rightdoor_pos not found in observation, using placeholder value.")
        ...
        if self.initial_base is None:
            ...
            print(f"[GoToCabinetHandlePolicyRight] Right handle detected at: {self.target_pos}")
            print(f"[GoToCabinetHandlePolicyRight] Creating approach command with waypoints: {handle_command['waypoints']}")
            ...
            if base_command:
                ...
                print(f"[GoToCabinetHandlePolicyRight] Target EE position: {self.target_ee_pos}")
        ...
        if self.state == 'move_base':
            ...
            if action is None:  # Base movement complete
                ...
                if action and 'base_pose' in action:
                    target_pose = action['base_pose']
                    print(f"[GoToCabinetHandlePolicyRight] Target base pose: [{target_pose[0]:.3f}, {target_pose[1]:.3f}, {target_pose[2]:.3f}]")
                # Hold arm at current position
                action['arm_pos'] = arm_pos.copy()
                action['arm_quat'] = arm_quat.copy()
                action['gripper_pos'] = self.gripper_open.copy()
        ...
        # State: move_arm
            ...
            angle = 2 * np.arccos(np.abs(dot))
            if pos_close and angle < np.deg2rad(5):
                print("[GoToCabinetHandlePolicyRight] Finished moving arm to right handle pose. Transitioning to grasp.")
                self.state = 'grasp'
            return action
        ...
        # State: grasp
            ...
            if np.allclose(gripper_pos, self.gripper_closed, atol=0.1):
                print("[GoToCabinetHandlePolicyRight] Finished grasping right handle. Transitioning to move_base_away.")
                self.state = 'move_base_away'
            return action
        ...
        # State: move_base_away (using simple direct movement)
            ...
            if np.allclose(base_pose, self.base_target_away, atol=0.01):
                print("[GoToCabinetHandlePolicyRight] Finished moving base away with right handle. Transitioning to done.")
                self.state = 'done'
            return action
