...
class MotionPlannerPolicy(Policy):
    ...
    def __init__(self, num_task_cubes=None):
        ...
        self.PUSH_ARM_OFFSET = 0.055  # 5cm in front of object for pushing
        ...
        #added for push primitive
        self.desired_arm_pos = None # Desired arm position for holding during base movement
        
        self.object_to_push_ids = [] #added for push primitive
        self.center_object_id = None #added for push primitive
        self.gripper_push_open_pos = 0.4 #gripper open position for pushing
    ...
    def _step(self, obs):
        ...
        if self.state == 'idle':
            ...
            target_objects = self.detect_objects_from_ground_truth(obs, select_object_id=2)
            if detected_objects:
                # Create pick command
                ...
                #    self.object_location[0],  # 50cm in X direction
                #    self.object_location[1],        # Same Y as object
                #    self.object_location[2]         # Same Z as object (table height)
                #])
                self.target_location = np.array([
                    target_objects[0][0],  # 50cm in X direction
                    target_objects[0][1],        # Same Y as object
                    target_objects[0][2]        # Same Z as object (table height)
                ])'''
            # Detect objects and plan new command
            ...
            if self.object_to_push_ids:
                ...
                # Calculate push angle (for gripper orientation later)
                push_angle = math.atan2(push_direction_2d_normalized[1], push_direction_2d_normalized[0])
                
                # Base stops BEHIND object (on reverse extension line)
                arm_reach = 0
                total_offset = self.get_end_effector_offset('push') + arm_reach # ~0.67m
                base_target_2d = self.object_location[:2] - push_direction_2d_normalized * total_offset 

                push_command = {
                    'primitive_name': 'push',
                    'waypoints': [base_pose[:2].tolist(), base_target_2d.tolist()],
                    'object_3d_pos': self.object_location.copy(),  # Store full 3D position
                    'target_3d_pos': self.target_location.copy(),#'object_id': next_object_id,
                    'push_angle': push_angle,
                    'push_direction': push_direction_2d_normalized
                }
                print(f"Pushing Object at: {self.object_location}")
                ...
                print(f"Creating push command with waypoints: {pick_command['waypoints']}")'''
                
                # Build base command and start moving
                base_command = self.build_base_command(push_command) #push_command if using push primitive
                if base_command:
                    self.current_command = push_command #push_command if using push primitive
                ...
                #return self.search_for_objects(obs) #original code
                ...
                if self.target_ee_pos is not None:
                    ...
                    if self.current_command['primitive_name'] == 'place':
                        if diff < 0.05:  # 0.2 cm tolerance (reduced from 10 cm)
                            ...
                            print("Base reached target, starting arm manipulation", self.current_command['primitive_name'])
                        ...
                        if diff-0< 0.05:  # 0.2 cm tolerance (reduced from 10 cm)#0.35
                            self.state = 'manipulating'
                            print("Base reached target, starting arm manipulation",self.current_command['primitive_name'])
                            #breakpoint()
                        else:
                            ...
                            self.state = 'idle'
                        #breakpoint()    
                    elif self.current_command['primitive_name'] == 'movebtwpush':
                        if diff < 0.05:  # 0.2 cm tolerance (reduced from 10 cm)
                            self.state = 'manipulating'
                            print("Base reached target, starting arm manipulation",self.current_command['primitive_name'])
                        else:
                        ...
                        print('Unknown primitive name', self.current_command['primitive_name'])
                else:
                    self.state = 'idle'
            else:
            ...
            if self.current_command['primitive_name'] == 'pick':
                ...
                if 'object_3d_pos' in self.current_command:
                    ...
                    object_3d_pos = self.current_command['object_3d_pos']
                    push_arm_target_pos = object_3d_pos.copy() #update for push primitive
                    push_arm_target_pos[:2] = push_arm_target_pos[:2] - self.current_command.get('push_direction') * self.PUSH_ARM_OFFSET #update for push primitive
                    # Calculate arm mount position in global frame (accounting for base rotation)
                    base_angle = base_pose[2]
                    cos_angle = math.cos(base_angle)
                    sin_angle = math.sin(base_angle)
                    ...
                    global_diff = np.array([
                        push_arm_target_pos[0] - arm_mount_global[0],
                        push_arm_target_pos[1] - arm_mount_global[1], 
                        push_arm_target_pos[2] + 0.25 - 0.48 - 0.053  # Object height + higher offset - arm mount height
                    ])
                ...
                if self.grasp_step == 0:
                    ...
                    # Move to next step after arm is positioned
                    if np.allclose(arm_pos_from_mount, object_relative_pos, atol=0.03):  # Tighter tolerance: 3cm
                        print(arm_pos_from_mount-object_relative_pos)
                        self.grasp_step = 1
                        print("Arm positioned above object, moving to lower approach")
                    ...
                    lower_pos[2] -= 0.08  # Start from lowered grasping position
                    lifted_pos = lower_pos.copy()
                    lifted_pos[2] += 0.015  # Lift by 28cm from grasping position (net +20cm from original)
                    action = {
                        'base_pose': base_pose.copy(),
                        'arm_pos': lifted_pos,  # Lift the object
                        'arm_quat': np.array([1.0, 1.0, 0.0, 0.0]),  # Inverted: gripper fingers pointing down toward ground
                    ...
                    '''target_3d_pos = np.array([
                        self.target_location[0],  # Target X
                        self.target_location[1],  # Target Y
                        lifted_pos[2]   # Object's Z (no height change)
                    ])'''
                    self.desired_arm_pos = lifted_pos.copy()
                    ...
                    if np.allclose(arm_pos_from_mount, lifted_pos, atol=0.01):  # 5cm tolerance
                        ...
                        base_command = self.build_base_command(movebtwpush_command)
                        if base_command:
                            self.current_command = movebtwpush_command
                            self.base_waypoints = base_command['waypoints']
                            self.target_ee_pos = base_command['target_ee_pos']  # Will be None for 'move'
                            self.current_waypoint_idx = 1
                            self.lookahead_position = None
                            self.state = 'moving'
                            self.grasp_step = 0
                            print(f"Starting base movement to target location at {self.target_location}")
                        else:
                ...
                print('move between push primitive')
                #breakpoint()
                
                high_pos = self.desired_arm_pos.copy()#arm_pos.copy()# 
                high_pos[2] += 0.1
                action = {
                    'base_pose': base_pose.copy(),
                    'arm_pos': high_pos,
                    'arm_quat': np.array([1.0, 1.0, 0.0, 0.0]),#obs['arm_quat'].copy(),#arm_quat.copy(),
                    'gripper_pos':np.array([0])  #gripper_pos.copy(),
                }
                print("Object already at target location")
                # Check if arm reached target position
                arm_pos_from_mount = arm_pos - np.array([self.ARM_MOUNT_OFFSET[0], self.ARM_MOUNT_OFFSET[1], 0])
                arm_error = np.linalg.norm(arm_pos_from_mount - high_pos)
                print(f"Lifting arm... error: {arm_error:.3f}m", high_pos, arm_pos)
                if arm_error < 0.05:  # 5cm tolerance - arm reached target
                   #breakpoint()
                   print("Arm lifted! Moving for pushing next object")
                   self.object_to_push_ids.pop(0)
                   self.desired_arm_pos[2] += 0.1
                   self.state = 'idle'
                return action
        action = {
            'base_pose': base_pose.copy(),
    ...
    def execute_base_movement(self, obs):
        ...
        if self.desired_arm_pos is not None:
            arm_pos_from_mount = self.desired_arm_pos
            
        else:
            arm_pos_from_mount = obs['arm_pos'] - np.array([self.ARM_MOUNT_OFFSET[0], self.ARM_MOUNT_OFFSET[1], 0])
        
        print(f"Arm pos from mount: {arm_pos_from_mount}")
        #breakpoint()
    ...
    def build_base_command(self, command):
        ...
        assert command['primitive_name'] in {'move', 'pick', 'place', 'toss', 'shelf', 'drawer', 'push', 'movebtwpush'}

        # Base movement only
        if command['primitive_name'] == 'move':
            return {'waypoints': command['waypoints'], 'target_ee_pos': None, 'position_tolerance': 0.1}
        # For push operation, waypoints are already pre-calculated (base on reverse extension line)
        # target_ee_pos should be object position (where arm needs to reach)
        if command['primitive_name'] == 'push':
            object_pos_2d= command['object_3d_pos'][:2].tolist()  # object position
            target_ee_pos = object_pos_2d - command.get('push_direction') * 0#0.35
            return {
                'waypoints': command['waypoints'],  # pass through, already calculated
