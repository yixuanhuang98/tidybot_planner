...
# Motion Planner generated plan for stacking cubes on table
class MotionPlannerTableStackPolicy(BaseAgent):
    ...
    TABLE_HEIGHT = 0.42  # Table surface height in meters
    OBJECT_HEIGHT_ON_TABLE = 0.48  # Height of objects on table (table + object radius)
    STACK_HEIGHT_OFFSET = 0.04  # 4cm above the target cube for stacking (reduced for table)

    # Manipulation parameters - adjusted for table environment
    ROBOT_BASE_HEIGHT = 0.48
    PICK_APPROACH_HEIGHT_OFFSET = 0.18  # Reduced from 0.25 for table work, was 0.15
    PICK_LOWER_DIST = 0.04  # Reduced from 0.09 for more precise table manipulation, was 0.06
    PICK_LIFT_DIST = 0.20   # Reduced from 0.28 for table work
    PLACE_APPROACH_HEIGHT_OFFSET = 0.08  # Reduced for table environment
    ...
    def __init__(self):
        ...
        print(f'Motion planner table stack policy initialized - ready to start automatically')
    ...
    def reset(self):
        ...
        print("Motion planner table stack policy reset - starting episode automatically")
    ...
    def _step(self, obs):
        ...
        if self.state == 'idle':
            ...
            if cube_info:
                ...
                # Set stack location on top of target cube (adjusted for table height)
            ...
            if self.current_command['primitive_name'] == 'pick':
                ...
                # Position arm above object and close gripper to grasp (adjusted for table height)
                object_3d_pos = self.current_command['object_3d_pos']
                # Calculate global position difference with table-adjusted approach height
                global_diff = np.array([
                    object_3d_pos[0] - base_pose[0],
                    object_3d_pos[1] - base_pose[1], 
                    object_3d_pos[2] + self.PICK_APPROACH_HEIGHT_OFFSET - self.ROBOT_BASE_HEIGHT  # Object height + table-optimized offset - base height
                ])
                ...
                print(f"Table pick: global_diff = {global_diff}")
                ...
                if self.grasp_state == PickState.APPROACH:
                    ...
                    print(f"Step 1: Positioning arm above table object with open gripper")
                    if np.allclose(arm_pos, target_arm_pos, atol=0.025):  # Slightly looser tolerance for table work: 2.5cm
                        self.grasp_state = PickState.LOWER
                        print("Arm positioned above table object, moving to lower approach")
                
                elif self.grasp_state == PickState.LOWER:
                    # Step 2: Lower gripper closer to object for precise grasping (adjusted for table)
                    target_arm_pos = object_relative_pos.copy()
                    target_arm_pos[2] -= self.PICK_LOWER_DIST  # Lower by reduced distance for table work
                    target_arm_quat = np.array([1.0, 0.0, 0.0, 0.0])  # Gripper down
                    target_gripper_pos = np.array([0.0])  # Gripper open

                    print(f"Step 2: Lowering gripper for table approach... target: {target_arm_pos[2]:.3f}, current: {arm_pos[2]:.3f}")
                    if np.allclose(arm_pos, target_arm_pos, atol=0.025):  # Tight tolerance for table: 2.5cm, was 1.5cm
                        self.grasp_state = PickState.GRASP
                        print("Gripper lowered to table grasping position, closing gripper")
                    ...
                    print(f"Step 3: Closing gripper on table... current position: {gripper_pos[0]:.3f}")
                    ...
                    if not hasattr(self, 'grasp_start_time'):
                        ...
                        print(f"Started table grasp attempt, initial gripper pos: {self.initial_gripper_pos:.3f}")
                    ...
                    if gripper_closed_enough or gripper_progress or grasp_timeout:
                        if gripper_closed_enough or gripper_progress:
                            print(f"Table grasp successful! Gripper pos: {gripper_pos[0]:.3f}, progress: {gripper_pos[0] - self.initial_gripper_pos:.3f}")
                        else:
                            print(f"Table grasp timeout reached, proceeding with current grip: {gripper_pos[0]:.3f}")
                        
                        self.grasp_state = PickState.LIFT
                        # Clean up tracking variables
                        delattr(self, 'grasp_start_time')
                        delattr(self, 'initial_gripper_pos')
                        print("Moving to table lift phase!")
                
                elif self.grasp_state == PickState.LIFT:
                    # Step 4: Lift object from the grasping position (adjusted for table)
                    ...
                    print(f"Step 4: Lifting table object... target height: {target_arm_pos[2]:.3f}, current: {arm_pos[2]:.3f}")
                    if np.allclose(arm_pos, target_arm_pos, atol=0.04):  # 4cm tolerance for table work
                        ...
                        if gripper_pos[0] > self.GRASP_FAILURE_THRESHOLD:
                            if self.grasp_retries < self.MAX_GRASP_RETRIES:
                                ...
                                print(f"Table grasp failed after lift (gripper pos: {gripper_pos[0]:.3f}). Retrying grasp, attempt {self.grasp_retries}/{self.MAX_GRASP_RETRIES}.")
                                self.grasp_state = PickState.APPROACH
                            else:
                                print(f"Table grasp failed after {self.MAX_GRASP_RETRIES} retries. Aborting.")
                                self.episode_ended = True
                                self.state = 'idle'
                        else:
                            print("Table object lifted successfully! Now moving to stack location on target cube.")
                            ...
                            if base_command:
                                ...
                                print(f"Starting base movement to table stack location at {self.stack_location}")
                            else:
                                print("Failed to build table place command")
                                self.episode_ended = True
                                self.state = 'idle'
                ...
                if cube_info:
                    ...
                    print(f"Updated precise table stack location: {precise_stack_location}")
                else:
                    # Fallback to original target if detection fails
                    target_3d_pos = self.current_command['target_3d_pos']
                    print("Using fallback table stack location")

                # Calculate positions for two-stage placement (adjusted for table)
                # Stage 1: Safe approach position (12cm above target cube for table work)
                safe_approach_height = 0.12  # 12cm above target cube for table
                global_diff_safe = np.array([
                    target_3d_pos[0] - base_pose[0],
                    target_3d_pos[1] - base_pose[1], 
                    target_3d_pos[2] + safe_approach_height - self.ROBOT_BASE_HEIGHT  # Target height + 12cm - base height
                ])
                
                # Stage 2: Final placement position (reduced offset for table precision)
                ...
                print(f"Table stacking: safe_approach_pos = {safe_approach_pos}")
                print(f"Table stacking: final_place_pos = {final_place_pos}")
                print(f"Target EE pos: {self.target_ee_pos}, Base pose: {base_pose}")
                print(f"Grasp state: {self.grasp_state}")
                
                if self.grasp_state == PlaceState.APPROACH:
                    # Step 1: Position arm at safe approach height (12cm above target cube)
                    target_arm_pos = safe_approach_pos
                    target_arm_quat = np.array([1.0, 0.0, 0.0, 0.0])  # Gripper down
                    target_gripper_pos = np.array([1.0])  # Gripper closed

                    print(f"Step 1: Moving to safe table approach position (12cm above target cube)")
                    ...
                    if np.allclose(arm_pos[:2], target_arm_pos[:2], atol=0.015):  # 1.5cm tolerance for table approach
                        self.grasp_state = PlaceState.LOWER_TO_PLACE
                        print("Reached safe table approach position, now lowering to placement height")
                
                elif self.grasp_state == PlaceState.LOWER_TO_PLACE:
                    # Step 2: Lower to final placement position (reduced offset above target cube)
                    target_arm_pos = final_place_pos
                    target_arm_quat = np.array([1.0, 0.0, 0.0, 0.0])  # Gripper down
                    target_gripper_pos = np.array([1.0])  # Gripper closed

                    print(f"Step 2: Lowering to final table placement position")
                    print(f"Target arm pos: {target_arm_pos}, Current arm pos: {arm_pos}")
                    print(f"Position error: {np.linalg.norm(arm_pos[:3] - target_arm_pos[:3]):.4f}m")
                    if np.allclose(arm_pos[:3], target_arm_pos[:3], atol=0.11):  # 0.8cm tolerance for precise table placement
                        self.grasp_state = PlaceState.RELEASE
                        print("Arm positioned at final table placement height, opening gripper")
                    ...
                    print(f"Step 3: Opening gripper to stack cube on table... current position: {gripper_pos[0]:.3f}")
                    if gripper_pos[0] < self.PLACE_SUCCESS_THRESHOLD:
                        print("Cube stacked successfully on table! Now lifting arm away.")
                        self.grasp_state = PlaceState.LIFT_AWAY
                
                elif self.grasp_state == PlaceState.LIFT_AWAY:
                    # Step 4: Lift arm away from stacked cubes with open gripper (reduced for table)
                    lifted_away_pos = final_place_pos.copy()
                    lifted_away_pos[2] += 0.10  # Lift arm 10cm above the placement position (reduced for table)
                    target_arm_pos = lifted_away_pos
                    target_arm_quat = np.array([1.0, 0.0, 0.0, 0.0])  # Gripper down
                    target_gripper_pos = np.array([0.0])  # Gripper open

                    print(f"Step 4: Lifting arm away from stacked table cubes... target height: {target_arm_pos[2]:.3f}, current: {arm_pos[2]:.3f}")
                    print(f"Position error: {np.linalg.norm(arm_pos - target_arm_pos):.4f}m")
                    if np.allclose(arm_pos[:3], target_arm_pos[:3], atol=0.025):  # 2.5cm tolerance for table work
                        print("Arm lifted away successfully! Table stacking task complete.")
    ...
    def detect_cubes_for_stacking(self, obs):
        ...
        for i in range(1, 4):
            ...
            if cube_key in obs:
                ...
                print(f"Detected table cube {i} at position: {cube_pos}")
            else:
                print(f"Warning: {cube_key} not found in observation")
        
        if len(cubes) < 2:
            print("Warning: Need at least 2 cubes for table stacking")
            return None
