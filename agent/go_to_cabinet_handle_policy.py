...
class GoToCabinetHandlePolicy(BaseAgent):
    """State machine: move base to approach pose, move arm to handle, grasp, move base away, hold"""
    
    def __init__(self):
        super().__init__()
        
        # Target handle pose and orientation (left door)
    ...
    def step(self, obs: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Execute cabinet handle manipulation"""
    ...
    def _initialize_base_approach(self, base_pose: np.ndarray):
        ...
        print(f"[GoToCabinetHandlePolicy] Handle at: {self.target_pos}")
    ...
    def _handle_move_arm(self, base_pose: np.ndarray, arm_pos: np.ndarray,
        ...
        if pos_close and quat_close:
            print("[GoToCabinetHandlePolicy] Arm at handle, starting grasp")
            self.state = 'grasp'
    ...
    def _handle_grasp(self, base_pose: np.ndarray, arm_pos: np.ndarray,
        ...
        if np.allclose(gripper_pos, self.gripper_closed, atol=0.1):
            print("[GoToCabinetHandlePolicy] Handle grasped, moving base away")
    ...
    def _handle_move_base_away(self, base_pose: np.ndarray, arm_pos: np.ndarray,
                              arm_quat: np.ndarray, gripper_pos: np.ndarray) -> Dict[str, Any]:
        """Handle moving base away while holding handle"""
        action = self.create_action(
        ...
        if self.check_position_reached(base_pose, self.base_target_away, tolerance=0.01):
            print("[GoToCabinetHandlePolicy] Base moved away, task complete")
            self.state = 'done'
