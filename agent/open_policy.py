...
class MotionPlannerPolicyCabinetMP(BaseAgent):
            ...
            if self.current_command['primitive_name'] == 'pick':
                if self.custom_grasp:
                    ...
                    if self.base_target_away is None:
                        self.base_target_away = np.array([base_pose[0] - 0.2, base_pose[1], base_pose[2]])
    ...
    def detect_objects_from_ground_truth(self, obs):
        """Detect objects using ground truth from MuJoCo simulation. For cabinet_scene, select the handle with least y value."""
        detected_objects = []
        ...
        if self.state == 'idle':
            ...
            if self.current_command['primitive_name'] == 'pick':
                if self.custom_grasp:
                    ...
                    # rotated_arm_quat = np.array([0.6532815, 0.6532815, 0.27059805, 0.27059805]) ## 45 degrees point downward.
                    if self.base_target_away is None:
                        self.base_target_away = np.array([base_pose[0] - 0.1, base_pose[1] - 0.05, base_pose[2]])
...
class CloseCabinetPolicy(BaseAgent):
    ...
    def __init__(self, cupboard_mode=False, custom_grasp=False, close_left_cabinet=False):
        """
        Initialize MotionPlannerPolicy
                ...
                if self.custom_grasp:
                    ...
                    if self.base_target_away is None:
                        if self.close_left_cabinet:
                            self.base_target_away = np.array([base_pose[0] + 0.15, base_pose[1] + 0.5, base_pose[2]])
                        else:
                            self.base_target_away = np.array([base_pose[0] + 0.15, base_pose[1] - 0.5, base_pose[2]])
